...
if (selects.length > 0) {
	var placeholder = selects.data("placeholder");
...
var input = $("<input type='text' readonly='readonly' placeholder='"+placeholder+"' class='input dummy' />")
...
if($(this).hasClass("active")){
	...
	strArr=reform(strArr,select.options[n].text);
	input.val(strArr.join(','));
}else{
	$(this).addClass('active');
	var n = $(this).index();
	select.selectedIndex = n;
	strArr.push(select.options[n].text);
	input.val(strArr.join(','));
}
console.log(strArr);
$(select).change();
...
$.fn.extend({ 
	locate: function (x, y) {
		...
		var scrollTopN = document.documentElement.scrollTop || window.pageYOffset || document.body.scrollTop;
		if ("Safari" == mb) {
			y += scrollTopN;
		}
	...
	locateCenter: function () {
		...
		return this.locate(
			...
			($(window).height() - this.height()) / 2 + scrollTopN
		);
	}
});
//lgj业务需求添加方法
function reform(arr,str){
	var arr1=[];
